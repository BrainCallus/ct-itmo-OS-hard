...
extern struct spinlock lnk_lock;
...
void output(pagetable_t pagetable, char *dots, int level) {
  for (int k = 0; k < 512; k++) {
    pte_t pte = pagetable[k];
    if (pte & PTE_V) {
      printf("%s%d: pte %p pa %p\n", dots, k, pte, PTE2PA(pte));
      int next_level = level - 1;
      if (level > 0) {
        uint64 child = PTE2PA(pte);
        pagetable_t pt = (pagetable_t)child;
        output(pt, printDots(next_level), next_level);
      }
    }
  }
}
...
int cow(uint64 va, struct proc *p) {
  ...
  if (*pte & PTE_C) {
    ...
    newpte = ((newpte | PTE_FLAGS(*pte)) | PTE_W) & ~PTE_C;
    *pte = newpte;
  } else {
    return -1;
  }
