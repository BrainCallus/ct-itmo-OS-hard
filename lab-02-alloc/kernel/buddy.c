...
void xor_bit(char *array, int index) {
  index >>= 1;
  char m = (1 << (index % 8));
  array[index / 8] ^= m;
}
...
void bd_print_vector(char *vector, int len) {
  ...
  lb = 0;
  for (int b = 0; b < len; b++) {
    if (last == get_bit(vector, b)) continue;
    if (last == 1) printf(" [%d, %d)", lb, b);
    lb = b;
    last = get_bit(vector, b);
  }
  if (lb == 0 || last == 1) {
    printf(" [%d, %d)", lb, len);
  }
...
void *bd_malloc(uint64 nbytes) {
  ...
  xor_bit(bd_sizes[k].alloc, blk_index(k, p));
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    xor_bit(bd_sizes[k - 1].alloc, blk_index(k - 1, p));
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    xor_bit(bd_sizes[k].alloc, bi);           // free p at size k
    if (get_bit(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
int bd_initfree_pair(int k, int bi, void *free_start, void *free_end) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (get_bit(bd_sizes[k].alloc, bi)) {
    free = BLK_SIZE(k);
